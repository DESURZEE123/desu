...
> 模块索引：**4**（与 `cooperation_history` / `project_base_info` / `implementation_plan` 同级 L1，调度顺序在实施方案之后）  
> 上游：用户传入的尽调项目全量参数  
...
> 本模块对各尽调系统**统一输出**（字段结构一致）；若入参无 `analysisResults` 或为空数组，整模块不输出。
...
6. 输出 moduleIndex=4 的结构化 JSON → 交由 L2 渲染
```
