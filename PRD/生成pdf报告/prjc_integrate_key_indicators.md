...
> 模块索引：**5**（与 `cooperation_history` / `project_base_info` / `implementation_plan` / `analysis_results` 同级 L1，调度顺序在其后）  
> 上游：用户传入的尽调项目全量参数  
...
6. 空值规范化为「—」；数值原样透传（不追加 %）
7. 输出 moduleIndex=5 的结构化 JSON → 交由 L2 渲染
```
