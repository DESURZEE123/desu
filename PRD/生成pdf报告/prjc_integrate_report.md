...
3. 若模块范围不明确，默认生成所有已配置模块（当前已配置：项目基本信息模块）。

### 二、Step 1 · 下发功能整合资源

针对每个待生成模块，下发对应的**功能整合资源**：
...
> 不同尽调系统的字段差异、模块特有逻辑，均在各模块功能整合资源中定义（详见后续各模块资源文档）。

**当前已规划模块：**

| 模块 | 资源名称 | 标识 | 说明 |
| ---- | -------- | ---- | ---- |
| 项目基本信息 | 项目基本信息 · 功能整合 | prjc_integrate_project_base_info | 待详细编写 |
| 其他模块 | — | — | 后续扩展 |
...
> 展示类型的具体样式规则（标签结构、内联样式、空值处理等）在**样式资源**中统一定义（待详细编写）。
...
| 资源名称 | 标识 | 层级 | 状态 |
| -------- | ---- | ---- | ---- |
| 报告自动生成 PDF 模块（总体架构 / 下发控制） | prjc_integrate_report | L0 | 
| 项目基本信息 · 功能整合 | prjc_integrate_project_base_info | L1 | 
| 统一规范样式 | prjc_style_render | L2 | 
