...
3. 若模块范围不明确，默认生成所有已配置模块（当前已配置：合作历史、项目基本信息、实施方案、AI分析结果、重点指标）。
...
| 5 | 重点指标 | 重点指标 · 功能整合 | `prjc_integrate_key_indicators` | `key_indicators` | 详见该资源文档；`prjc-flr` / `prjc-si` 整模块不输出 |
| — | 其他模块 | — | — | — | 后续扩展 |

> 调度顺序：合作历史 → 项目基本信息 → 实施方案 → AI分析结果 → 重点指标。汇总 HTML 时按此顺序拼接各模块片段。
...
- `moduleIndex`：供 L0 排序拼接；合作历史 → 项目基本信息 → 实施方案 → AI分析结果 → 重点指标
